  ...
  try {
    ...
    try {
      ...
      if (data) {
        ...
        if (hlsUrl) {
          // For downloads, provide the direct URL so Vega App can download the HLS stream
          // For streaming, proxy the URL to bypass CORS/geo restrictions
          const streamUrl = isDownload ? hlsUrl : `${proxyUrlBase}?url=${encodeURIComponent(hlsUrl)}`;
    ...
    try {
      ...
      if (dubData) {
        ...
        if (dubUrl) {
          ...
          const dubStreamUrl = isDownload ? dubUrl : `${proxyUrlBase}?url=${encodeURIComponent(dubUrl)}`;
